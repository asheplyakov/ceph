...
uint64_t parse_rbd_default_features(CephContext* cct) 
{
  int ret = 0;
  uint64_t value = 0;
  auto features = cct->_conf->get_val("rbd_default_features");
  const std::string* featuresp = boost::get<std::string>(&features);
  assert(featuresp);
  try {
    value = boost::lexical_cast<decltype(value)>(*featuresp);
    ...
    boost::split(strs, *featuresp, boost::is_any_of(","));
